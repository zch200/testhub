...
| 2026-03-08 | v2.1.0 | 优化 | **测试计划执行体验优化**：(1) 计划状态流转改为可逆（draft ⇄ in_progress ⇄ completed → archived），每次变更前弹窗确认；(2) 新增 `plan_case_remarks` 表，备注改为多条追加模式，删除 `plan_cases.remark` 字段；(3) 计划用例表格精简为仅保留勾选框、标题、状态列，状态下拉直接生效；(4) 新增用例执行侧边抽屉，整合用例详情、状态切换、多条备注、变更历史和逐条导航；(5) 移除独立的历史 Sheet 弹窗（合并到侧边抽屉）；(6) 批量操作精简为状态下拉 + 更新按钮 | 功能描述-7.2/7.3/10、数据模型、技术实现-前端、验收标准 | - |
| 2026-03-08 | v2.1.1 | 优化 | **执行抽屉交互优化**：(1) 计划概览进度百分比改为通过率（通过数/总数），不再以执行完成率计算；(2) 抽屉状态切换从按钮组改为下拉选择器，切换后 toast 提示并 2 秒后自动进入下条用例；(3) 抽屉区域结构从四区域改为头部 + 两个 Tab（「用例 & 备注」合并为单 Tab，「变更历史」独立 Tab） | 功能描述-7.3/7.4、技术实现-前端、验收标准 | - |
...
**流转规则：**
- **前进**：draft → in_progress → completed → archived
- **回退**：completed → in_progress（重新执行）、in_progress → draft（回到草稿）
- **终态**：archived 不可逆，不支持回退
- **v2.1.0 新增**：每次状态变更前弹出确认弹窗，显示"确认将计划从 [当前状态] 变更为 [目标状态]？"，防止误触

**前端交互：**
- 测试计划列表页显示所有计划及其状态
- 计划详情页顶部显示当前状态和状态操作按钮
- **前进按钮**：
  - draft → 显示「开始执行」
  - in_progress → 显示「完成计划」
  - completed → 显示「归档」
...
- **v2.1.0 新增 — 回退按钮**：
  - in_progress → 显示「回到草稿」
  - completed → 显示「重新执行」
  - draft / archived → 无回退按钮
- 点击任何状态操作按钮均弹出确认弹窗，用户确认后才执行变更

**API 支持：**
- `PUT /api/v1/plans/:id` - 更新计划状态

#### 7.3 执行测试和标记状态（前端核心功能）✅（修改于 v2.1.0）
...
|---|---|
| 勾选框 | 用于批量操作 |
| 标题 | 用例标题，**点击打开用例执行侧边抽屉** |
| 状态 | 执行状态下拉框，**选择后直接调用 API 更新**，无需点击保存 |

**v2.1.0 移除的列**：~~备注~~、~~原因说明~~、~~版本~~、~~操作（保存/历史按钮）~~

**用例执行侧边抽屉（修改于 v2.1.1）：**

点击用例标题打开侧边抽屉，结构为**头部 + 两个 Tab**：

**头部**：
- 用例标题 + 上一条/下一条导航按钮（含当前位置指示，如 3/12）
- **v2.1.1 修改**：状态切换使用**下拉选择器**（pending / passed / failed / blocked / skipped），选择后直接调用 API 更新
- **v2.1.1 新增**：切换状态后显示 toast 提示「将自动进入下条用例」，**2 秒后自动切换到下条用例**；若当前已是最后一条则不自动跳转

**Tab 1 —「用例 & 备注」（v2.1.1 合并）**：
- **上半部分 — 用例内容（只读）**：优先级、类型、标签、前置条件、执行步骤表格（step 模式）或文本内容（text 模式），供执行参考
- **下半部分 — 备注区域**：
  - 已有备注列表（按时间倒序），每条显示内容和创建时间
  - 底部输入框 + 「添加备注」按钮，追加新备注（不覆盖旧备注）
  - 备注用于记录失败原因、截图链接、补充说明等

**Tab 2 —「变更历史」**：
- 该用例的状态变更时间线（原独立的历史 Sheet 合并至此）

**批量标记状态（v2.1.0 精简）：**
- 勾选多个用例
- 顶部操作栏显示：执行状态下拉框 + 「更新已选」按钮
- 选择状态后点击按钮，批量更新
- **v2.1.0 移除**：~~原因说明输入框~~

**API 支持：**
- `GET /api/v1/plans/:planId/cases` - 获取计划中的用例列表（支持按状态筛选、分页排序）
- `PUT /api/v1/plans/:planId/cases/:planCaseId` - 更新执行状态
- `POST /api/v1/plans/:planId/cases/batch-status` - 批量更新状态
- `GET /api/v1/plans/:planId/cases/:planCaseId/history` - 获取单个用例的状态变更历史
- `GET /api/v1/plans/:planId/history` - 获取计划级状态变更时间线
- **v2.1.0 新增**：`POST /api/v1/plans/:planId/cases/:planCaseId/remarks` - 添加备注
- **v2.1.0 新增**：`GET /api/v1/plans/:planId/cases/:planCaseId/remarks` - 获取备注列表

#### 7.4 查看计划统计（修改于 v2.1.1）
...
- 显示文字统计信息
- **v2.1.1 修改**：计划概览卡片中的**进度百分比 = 通过数 / 总数**（即通过率），而非已执行数 / 总数。这确保只有真正通过的用例才计入进度，全部失败时进度为 0% 而非 100%
...
- **原因备注**（reason_note）：可选的文字说明（保留在历史表中，用于审计）
- **操作者**（actor）：执行操作的人（来自 `x-testhub-operator` 请求头）
- **来源**（source）：api / ui / system（来自 `x-testhub-source` 请求头）

**展示方式（v2.1.0 修改）：**
- 计划详情页底部保留"计划时间线"（Plan Timeline），展示该计划下所有用例的状态变更记录
- **v2.1.0 修改**：单个用例的变更历史不再使用独立的历史 Sheet 弹窗，改为集成到用例执行侧边抽屉的底部区域
- ~~每条计划用例可点击"History"按钮，在侧边抽屉中查看该用例的完整状态变更历史~~（v2.1.0 移除独立入口，合并到执行抽屉）
...
- 作为 **测试执行人员**，我想要为执行结果添加多条备注，以便记录失败原因、补充说明和后续跟进（v2.1.0 修改）
- 作为 **测试执行人员**，我想要控制测试计划的状态流转（支持回退），以便在误操作时能恢复到之前的状态（v2.1.0 修改）
- **（v2.1.0 新增）** 作为 **测试执行人员**，我想要在侧边抽屉中查看用例详情并逐条执行，以便专注于当前用例的测试而不被列表干扰
...
- **主要组件**：
  ...
  - `PlanStatusControl` - 测试计划状态流转控制（v2.1.0 修改：支持回退按钮 + 确认弹窗）
  - `PlanCaseExecutionDrawer` - 用例执行侧边抽屉（v2.1.0 新增，v2.1.1 修改：状态下拉选择器 + 自动进入下条 + 「用例 & 备注」/「变更历史」双 Tab）
...
- **关键交互**（v2.1.0 修改）：
  - 用例列表点击行展开详情（只读查看）
  - 目录树点击节点筛选用例
  - 测试计划状态流转按钮，支持前进和回退，每次弹窗确认（v2.1.0 修改）
  - 测试计划用例表格精简为勾选框 + 标题 + 状态列，状态下拉直接生效（v2.1.0 修改）
  - 点击用例标题打开执行侧边抽屉，可查看详情、切换状态、添加多条备注、查看历史（v2.1.0 新增，v2.1.1 修改交互）
  - 执行侧边抽屉支持逐条导航，切换状态后自动进入下条用例（v2.1.1 修改）
  - 批量选择用例进行批量标记状态（精简为状态下拉 + 更新按钮）
  - 计划详情页展示完整状态变更时间线
  - 侧边栏项目切换器切换当前项目
...
**12 张表（v2.1.0 新增 plan_case_remarks）：**
...
| plan_cases | 计划-用例执行表 | plan_id, case_id(onDelete: restrict), case_version_id(onDelete: restrict), execution_status(pending/passed/failed/blocked/skipped), executed_at; UNIQUE(plan_id, case_id)。**v2.1.0 删除 remark 字段**，备注改由 plan_case_remarks 表管理 |
| plan_case_remarks | 计划用例备注表（v2.1.0 新增） | id(PK), plan_case_id(FK, onDelete: cascade), content(text, 最多 2000 字符), created_at |
...
- [ ] **（v2.1.0 新增）** 可以将计划从 in_progress 回退到 draft（回到草稿）
- [ ] **（v2.1.0 新增）** 可以将计划从 completed 回退到 in_progress（重新执行）
- [ ] **（v2.1.0 新增）** archived 状态不可回退
- [ ] **（v2.1.0 新增）** 每次状态变更前弹出确认弹窗，用户确认后才执行

#### 测试执行与状态标记（核心功能，v2.1.0 修改）
- [ ] **（v2.1.0 修改）** 计划用例表格仅显示勾选框、标题、状态三列
- [ ] **（v2.1.0 修改）** 状态下拉框选择后直接调用 API 更新，无需点击保存
- [ ] **（v2.1.0 新增）** 点击用例标题打开执行侧边抽屉
- [ ] **（v2.1.1 修改）** 侧边抽屉头部显示用例标题、状态下拉选择器、上一条/下一条导航按钮
- [ ] **（v2.1.1 新增）** 切换状态后显示 toast 提示，2 秒后自动进入下条用例（最后一条时不自动跳转）
- [ ] **（v2.1.1 修改）** 侧边抽屉分为「用例 & 备注」和「变更历史」两个 Tab
- [ ] **（v2.1.0 新增）** 「用例 & 备注」Tab 上半部分显示用例内容（前置条件、步骤/文本），只读
- [ ] **（v2.1.0 新增）** 「用例 & 备注」Tab 下半部分显示备注列表（按时间倒序）+ 添加备注输入框
- [ ] **（v2.1.0 新增）** 可以在侧边抽屉中添加新备注（追加模式，不覆盖旧备注）
- [ ] **（v2.1.0 新增）** 「变更历史」Tab 显示该用例的状态变更时间线
- [ ] 支持勾选多个用例，批量标记状态
- [ ] **（v2.1.0 修改）** 批量操作精简为状态下拉框 + 「更新已选」按钮
- [ ] **（v2.1.1 修改）** 计划详情页进度百分比 = 通过数 / 总数（通过率），而非已执行数 / 总数
...
### 计划用例状态变更历史（v2.1.0 修改）
- [ ] 每次状态变更自动记录历史（包含 actor、source、reasonType）
- [ ] 可以在状态更新时附带 reasonNote（API 层保留）
- [ ] 计划详情页展示计划级状态变更时间线
- [ ] **（v2.1.0 修改）** 单个用例的变更历史集成到执行侧边抽屉中展示（不再使用独立的历史 Sheet）
- [ ] 批量更新状态时同样记录历史

### 计划用例备注（v2.1.0 新增）
- [ ] **（v2.1.0 新增）** 可以为计划用例添加多条备注（追加模式）
- [ ] **（v2.1.0 新增）** 备注列表按时间倒序展示
- [ ] **（v2.1.0 新增）** 备注通过独立 API 管理（`POST` 添加 / `GET` 列表）
- [ ] **（v2.1.0 新增）** 删除计划用例时级联删除其备注

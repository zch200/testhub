...
**活跃需求：**
- [TestHub - 本地优先测试管理系统](requirements/core/test-management-system.md) - v2.1.1（开发中）
  - 人机协作测试管理工具：Agent 通过 API 管理项目/用例/计划，人类通过 UI 执行测试和标记结果
  - v2.1.1：执行抽屉交互优化 — 进度改为通过率、状态下拉+自动进入下条、用例&备注合并为单 Tab
...
12 张表：projects, libraries, directories, cases, case_steps, tags, case_tags, case_versions, plans, plan_cases, plan_case_remarks, plan_case_status_histories
...
### v2.1.1（开发中） - 2026-03-08
- 计划概览进度百分比改为通过率（通过数/总数）
- 执行抽屉状态切换从按钮组改为下拉选择器，切换后自动进入下条用例
- 执行抽屉「用例」和「备注」合并为单 Tab，变更历史独立 Tab

### v2.1.0 - 2026-03-08
- 测试计划状态流转改为可逆（draft ⇄ in_progress ⇄ completed → archived），每次变更前弹窗确认
- 新增 `plan_case_remarks` 表，备注改为多条追加模式，删除 `plan_cases.remark` 字段
- 计划用例表格精简为勾选框 + 标题 + 状态列，状态下拉直接生效
- 新增用例执行侧边抽屉（整合用例详情、状态切换、备注、历史、逐条导航）
- 批量操作精简，移除独立历史 Sheet
